...
'''
if len(sys.argv) <4  or not sys.argv[1].startswith('/nfs/site') or not sys.argv[2].startswith('/nfs/site'):
    print("sample command is -->  python compare_drops.py <last_drop_path> <current_drop_path> 0")
...
if project_switch == 1:
    last_drop_include = os.path.join(last_drop_smedia,'source/includes/src')
    current_drop_include = os.path.join(current_drop_smedia,'source/includes/src')
else:
    last_drop_include = os.path.join(last_drop_smedia,'source/includes')
    current_drop_include = os.path.join(current_drop_smedia,'source/includes')
...
if project_switch == 1:
    #Finding the differnces in libs folder data between current and last drops
    log_file = 'diff_data_libs.txt'
    if os.path.isfile(os.path.join(os.getcwd(), log_file)):
    	os.remove(os.path.join(os.getcwd(), log_file))
    	print("deleted old "+log_file)

    last_drop_libs = os.path.join(last_drop_smedia,'source/includes/libs')
    current_drop_libs = os.path.join(current_drop_smedia,'source/includes/libs')
    print("checking the differenced files in libs folder")
    if os.path.isfile(os.path.join(os.getcwd(), log_file)):
        os.system("diff -r " + current_drop_libs + " " + last_drop_libs + " >> "+log_file)
    else:
        os.system("diff -r " + current_drop_libs + " " + last_drop_libs + " > "+log_file)



#Finding the differnces in rtl folder data between current and last drops
log_file = 'diff_data_rtl.txt'
if os.path.isfile(os.path.join(os.getcwd(), log_file)):
	os.remove(os.path.join(os.getcwd(), log_file))
	print("deleted old "+log_file)

last_drop_rtl = os.path.join(last_drop_smedia, 'source/rtl/src')
current_drop_rtl = os.path.join(current_drop_smedia,'source/rtl/src')
...
if project_switch == 1:
    f = open('diff_data_libs.txt','r')
    lines = f.readlines()
    f.close()

    f = open('diff_libs_full_paths.txt','w')
    f1 = open('diff_libs.txt','w')
    for line in lines:
        if line.startswith('diff -r'):
            line = line.strip().split(" ")
            f.write(line[2]+"\n")
...
for line in lines:
    ...
    last_drop_file = line.replace(current_drop_smedia,last_drop_smedia)
...
if project_switch == 1:
    f = open('diff_libs_full_paths.txt','r')
    lines = f.readlines()
    f.close()
    lines = [x.strip() for x in lines]
    res = open('diff_libs_rm_timedate.txt','w')
    for line in lines:
        current_drop_file = line
        last_drop_file = line.replace(current_drop_smedia,last_drop_smedia)
        if os.path.isfile(last_drop_file):

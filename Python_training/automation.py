...
if not os.path.isdir(os.path.join(os.getcwd(),folder)):
	...
	cmd = "git clone $GK_XE2LPM_HEAD "+ folder
	os.system(cmd)
...
elab_command = "gnr gen_rtl --autorpt --no-grip --context MEDIA_LPM_1VD1VE_SD -T o3c.vpi.sipdfx.gtsynth.goldenrpt.default64 -d sm -- --optimus && gnr elab --no-genrtl --context MEDIA_LPM_1VD1VE_SD -T o3c.vpi.sipdfx.gtsynth.goldenrpt.default64 -d sm -- --partcomp"
#os.system(elab_command)
print("elab is completed")
print("running export command")
export_command = 'gnr export_ip --context MEDIA_LPM_1VD1VE_SD -T o3c.vpi.sipdfx.gtsynth.goldenrpt.default64 --no-elab --gen-struct --collaterals-yaml cfg/export_ip/sm/collaterals.yaml --collaterals-copy -d sm -- --disable-test val rtl_questa'
...
#current_drop = '/nfs/site/disks/mtl_pipesm_aravind_emulation_model/abhishek/prashant_ww51p5/bld/sm/MEDIA_LPM_1VD1VE_SD.o3c.vpi.sipdfx.goldenrpt.default64/export_ip/debug_u2c/ip_package'
print(current_drop)
if not os.path.isdir(current_drop):
	sys.exit("export not done properly")


last_drop = reference_path
print("last drop is",last_drop)
#comparing the 2 fodlers and copying from the latest folder and putting them in different fodler and crfeating the zip folder of that folder

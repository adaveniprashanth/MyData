...
if 0:#printing Executions_stage_results
    for i in Executions_stage_results:
        print(i)


#test area tables creation(separate harware and RM tables)
if 0:
...
if 1:
    ...
    #merging 2 dictionaries for combining hardware and RM(software) tests
    ...
    combined_execution_stage_results = []
    def combine_dictionaries():
        result = mergeDictionary(optimized['2x4x8 Emulation Compare'], optimized['2x4x8 Renderfulsim'])
        test_names_combined = '2x4x8 Emulation Compare'+' and '+'2x4x8 Renderfulsim'
        combined_execution_stage_results.append([test_names_combined,result])
        
        result = mergeDictionary(optimized['2x4x8 Emulation Lockup'], optimized['2x4x8 Rendermill'])
        test_names_combined = '2x4x8 Emulation Lockup'+' and '+'2x4x8 Rendermill'
        combined_execution_stage_results.append([test_names_combined,result])
        
        result = mergeDictionary(optimized['Lockup_Method'], optimized['Fulsim_Tests'])
        test_names_combined = 'Lockup_Method'+' and '+'Fulsim_Tests'
        combined_execution_stage_results.append([test_names_combined,result])

    combine_dictionaries()
    
    def table_data_creation_for_combined_data(data):
        HW_total = HW_passed = HW_failed = HW_warned = HW_error = HW_remained = 0
        RM_total = RM_passed = RM_failed = RM_warned = RM_error = RM_remained = 0
        for k,v in data.items():
            f.write(f"<tr><td>{k}</td><td>{v[0]}</td><td>{v[1]}</td><td>{v[2]}</td><td>{v[3]}</td><td>{v[4]}</td><td>{v[5]}</td><td>{v[6]}</td><td>{v[7]}</td><td>{v[8]}</td><td>{v[9]}</td><td>{v[10]}</td><td>{v[11]}</td>")
            f.write(f"<td id=row2perecentage style='background-color:#{create_color_value(v)}'>{str((v[1]/v[0])*100)[:5]}%</td><td id=row2perecentage style='background-color:#{create_color_value(v)}'>{str((v[7]/v[6])*100)[:5]}%</td></tr>")
            HW_total+=v[0]
            HW_passed+=v[1]
            HW_failed+=v[2]
            HW_warned+=v[3]
            ...
            RM_error+=v[10]
            RM_remained+=v[11]
        return HW_total,HW_passed,HW_failed,HW_warned,HW_error,HW_remained,RM_total,RM_passed,RM_failed,RM_warned,RM_error,RM_remained


    for i in combined_execution_stage_results:
        print(i)
        table_heading_creation(i[0])
        f.write(table_start)
        f.write('''<tr><th>Units</th><th>Total</th><th>HW Pass</th><th>HW Fail</th><th>HW Warn</th><th>HW Error</th><th>HW Remain</th>
                <th>Total</th><th>RM Pass</th><th>RM Fail</th><th>RM Warn</th><th>RM Error</th><th>RM Remain</th><th>HW Pass %</th><th>RM Pass %</th></tr>''')
        returned = table_data_creation_for_combined_data(i[1])
        f.write(f"<tr><td style='background-color:#B6B6A8'>All</td><td style='background-color:#B6B6A8'>{returned[0]}</td><td style='background-color:#B6B6A8'>{returned[1]}</td><td style='background-color:#B6B6A8'>{returned[2]}</td><td style='background-color:#B6B6A8'>{returned[3]}</td><td style='background-color:#B6B6A8'>{returned[4]}</td><td style='background-color:#B6B6A8'>{returned[5]}</td><td style='background-color:#B6B6A8'>{returned[6]}</td><td style='background-color:#B6B6A8'>{returned[7]}</td>")
        f.write(f"<td style='background-color:#B6B6A8'>{returned[8]}</td><td style='background-color:#B6B6A8'>{returned[9]}</td><td style='background-color:#B6B6A8'>{returned[10]}</td><td style='background-color:#B6B6A8'>{returned[11]}</td><td id=row2perecentage style='background-color:#{create_color_value(returned[0:2])}'>{str((returned[1]/returned[0])*100)[:5]}%</td><td id=row2perecentage style='background-color:#{create_color_value(returned[6:8])}'>{str((returned[7]/returned[6])*100)[:5]}%</td></tr>")
        print(returned)
        f.write(table_close)
